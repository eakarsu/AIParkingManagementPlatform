...
// Gap (Non-AI): Limited customer self
// Limited customer self-service (no end-user permit purchase flow visible)
import React, { useState } from 'react';

export default function GapLimitedCustomerSelfPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-customer-self/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Limited customer self</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Limited customer self-service (no end-user permit purchase flow visible)</p>

...
// Custom Feature: Resident permit fraud detection
// Resident permit fraud detection: Multi-modal analysis (address changes, duplicate permits, unusual vehicle swaps); flag suspicious patterns
import React, { useState } from 'react';

export default function CFResidentPermitFraudDetectionPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-resident-permit-fraud-detection/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Resident permit fraud detection</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Resident permit fraud detection: Multi-modal analysis (address changes, duplicate permits, unusual vehicle swaps); flag suspicious patterns</p>

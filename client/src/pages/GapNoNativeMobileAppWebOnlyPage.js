...
// Gap (Non-AI): No native mobile app (web only
// No native mobile app (web only — some mobile keywords but no React Native)
import React, { useState } from 'react';

export default function GapNoNativeMobileAppWebOnlyPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-native-mobile-app-web-only/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No native mobile app (web only</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No native mobile app (web only — some mobile keywords but no React Native)</p>

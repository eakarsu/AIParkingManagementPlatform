...
// Gap (Non-AI): Limited multi
// Limited multi-tenant facility operator support
import React, { useState } from 'react';

export default function GapLimitedMultiPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-multi/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Limited multi</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Limited multi-tenant facility operator support</p>

...
// Gap (AI): Security without '/intrusion
// Security without /intrusion-detect (unusual entry attempts, loitering patterns)
import React, { useState } from 'react';

export default function GapSecurityWithoutIntrusionPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-security-without-intrusion/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Security without '/intrusion</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Security without /intrusion-detect (unusual entry attempts, loitering patterns)</p>

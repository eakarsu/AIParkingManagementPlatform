...
// Gap (Non-AI): No integrations with ride
// No integrations with ride-sharing (Uber, Lyft) for demand correlation
import React, { useState } from 'react';

export default function GapNoIntegrationsWithRidePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-integrations-with-ride/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No integrations with ride</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No integrations with ride-sharing (Uber, Lyft) for demand correlation</p>

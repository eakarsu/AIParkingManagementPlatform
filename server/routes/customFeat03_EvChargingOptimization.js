...
// Predict demand; pre-cool/warm chargers; recommend user charging times based on grid demand; integrate with utility demand response programs
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIParkingManagementPlatform / cf-ev-charging-optimization',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_ev_charging_optimization`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-ev-charging-optimization feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'EV charging optimization'.
Context: Predict demand; pre-cool/warm chargers; recommend user charging times based on grid demand; integrate with utility demand response programs
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-ev-charging-optimization/run error:', err);

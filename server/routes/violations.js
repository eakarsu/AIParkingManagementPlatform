...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const [result, count] = await Promise.all([
      db.query(`SELECT v.*, f.name as facility_name FROM violations v JOIN facilities f ON v.facility_id = f.id ORDER BY v.issued_at DESC LIMIT $1 OFFSET $2`, [limit, offset]),

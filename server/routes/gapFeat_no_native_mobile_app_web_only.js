...
// Gap Feature: No native mobile app (web only
// No native mobile app (web only — some mobile keywords but no React Native)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIParkingManagementPlatform / gap-no-native-mobile-app-web-only',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No native mobile app (web only'.\nProject: AIParkingManagementPlatform.\nContext: No native mobile app (web only — some mobile keywords but no React Native)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-native-mobile-app-web-only', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-native-mobile-app-web-only', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

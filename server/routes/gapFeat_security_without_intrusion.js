...
// Gap Feature: Security without \'/intrusion
// Security without /intrusion-detect (unusual entry attempts, loitering patterns)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIParkingManagementPlatform / gap-security-without-intrusion',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Security without \'/intrusion'.\nProject: AIParkingManagementPlatform.\nContext: Security without /intrusion-detect (unusual entry attempts, loitering patterns)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['security-without-intrusion', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'security-without-intrusion', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

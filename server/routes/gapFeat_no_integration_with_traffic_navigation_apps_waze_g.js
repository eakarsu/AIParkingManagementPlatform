...
// Gap Feature: No integration with traffic/navigation apps (Waze, Google Maps for supply signaling)
// No integration with traffic/navigation apps (Waze, Google Maps for supply signaling)
// Project: AIParkingManagementPlatform
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIParkingManagementPlatform / gap-no-integration-with-traffic-navigation-apps-waze-g',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No integration with traffic/navigation apps (Waze, Google Maps for supply signaling)'.\nProject: AIParkingManagementPlatform.\nContext: No integration with traffic/navigation apps (Waze, Google Maps for supply signaling)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-integration-with-traffic-navigation-apps-waze-g', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-integration-with-traffic-navigation-apps-waze-g', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-integration-with-traffic-navigation-apps-waze-g']
    );

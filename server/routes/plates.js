...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const [result, count] = await Promise.all([
      db.query(`SELECT p.*, f.name as facility_name FROM plate_records p JOIN facilities f ON p.facility_id = f.id ORDER BY p.captured_at DESC LIMIT $1 OFFSET $2`, [limit, offset]),
      db.query('SELECT COUNT(*) FROM plate_records'),
    ]);

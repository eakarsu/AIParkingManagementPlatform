...
// Multi-modal analysis (address changes, duplicate permits, unusual vehicle swaps); flag suspicious patterns
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIParkingManagementPlatform / cf-resident-permit-fraud-detection',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_resident_permit_fraud_detection`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-resident-permit-fraud-detection feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Resident permit fraud detection'.
Context: Multi-modal analysis (address changes, duplicate permits, unusual vehicle swaps); flag suspicious patterns
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-resident-permit-fraud-detection/run error:', err);

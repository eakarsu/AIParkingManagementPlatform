...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const [result, count] = await Promise.all([
      db.query(`SELECT p.*, f.name AS facility_name FROM permits p LEFT JOIN facilities f ON p.facility_id = f.id ORDER BY p.permit_number LIMIT $1 OFFSET $2`, [limit, offset]),

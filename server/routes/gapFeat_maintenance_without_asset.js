...
// Gap Feature: Maintenance without \'/asset
// Maintenance without /asset-lifecycle-predict (predict end-of-life for sensors, payment machines)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIParkingManagementPlatform / gap-maintenance-without-asset',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Maintenance without \'/asset'.\nProject: AIParkingManagementPlatform.\nContext: Maintenance without /asset-lifecycle-predict (predict end-of-life for sensors, payment machines)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['maintenance-without-asset', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'maintenance-without-asset', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

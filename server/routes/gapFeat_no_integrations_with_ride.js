...
// Gap Feature: No integrations with ride
// No integrations with ride-sharing (Uber, Lyft) for demand correlation
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIParkingManagementPlatform / gap-no-integrations-with-ride',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No integrations with ride'.\nProject: AIParkingManagementPlatform.\nContext: No integrations with ride-sharing (Uber, Lyft) for demand correlation\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-integrations-with-ride', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-integrations-with-ride', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

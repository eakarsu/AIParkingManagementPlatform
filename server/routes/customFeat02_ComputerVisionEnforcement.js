...
// IP cameras with plate recognition; auto-generates citations for expired permits or disabled placard misuse; reduces manual citation workload
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIParkingManagementPlatform / cf-computer-vision-enforcement',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_computer_vision_enforcement`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-computer-vision-enforcement feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Computer vision enforcement'.
Context: IP cameras with plate recognition; auto-generates citations for expired permits or disabled placard misuse; reduces manual citation workload
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-computer-vision-enforcement/run error:', err);

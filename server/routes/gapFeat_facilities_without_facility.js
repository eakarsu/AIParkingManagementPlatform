...
// Gap Feature: Facilities without \'/facility
// Facilities without /facility-audit-recommend (AI inspects photos/logs; suggests upgrades)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIParkingManagementPlatform / gap-facilities-without-facility',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Facilities without \'/facility'.\nProject: AIParkingManagementPlatform.\nContext: Facilities without /facility-audit-recommend (AI inspects photos/logs; suggests upgrades)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['facilities-without-facility', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'facilities-without-facility', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
